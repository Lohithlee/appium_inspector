...
`;
  ...
  codeFor_findAndAssign(strategy, locator, localVar, isArray, custom_operation) {
    let suffixMap = {
...
<>
  <tag>${localVar}</tag>
  <id_type>${suffixMap[strategy]}</id_type>
  <value>${locator}</value>
  <operation>${custom_operation}</operation>
  ...
  <tag>${localVar}</tag>
  <id_type>${suffixMap[strategy]}</id_type>
  <value>${JSON.stringify(locator)}</value>
  <operation>${custom_operation}</operation>
      `;
    }
  ...
  //   return  '<operation>click</operation>';
  // }
  ...
  codeFor_sendKeys(varName, varIndex, text) {
    return `<text_value>${text}</text_value>\n</>\n`;
  }
...
DotNetNUnitFramework.readableName = 'SAFE xml';
